...
* **实时进度显示**：任务列表中显示转码进度、输出文件大小及体积变化百分比。
...
* 在弹出对话框中设置输出格式、视频/音频参数。
* 点击 **确定** 将任务加入队列，或 **确定并开始** 立即执行。

### 任务管理

* **状态列**：显示任务状态（等待中/转码中/完成/错误）及进度条。
* **操作按钮**：每个任务右侧有“删除”按钮。
...
* 输出文件默认保存在源文件目录，文件名为 `原文件名\_时间戳.扩展名`。
...
A: 确保你的显卡是 NVIDIA，并安装了最新的显卡驱动。GPU 加速仅支持 H.264 和 H.265 编码（对应 `h264\_nvenc` / `hevc\_nvenc`
